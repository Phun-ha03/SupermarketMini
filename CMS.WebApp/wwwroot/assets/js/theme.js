...
var chatInit = function chatInit() {
  ...
  if ($chatEmojiarea) {
    $chatEmojiarea.setAttribute("placeholder", "Nhập câu hỏi của bạn...");
      ...
      var options = merge(
        {
          ...
          maxFilesize: 20,
          acceptedFiles:
            ".zip,.rar,.docx,.doc,.xlsx,.xls,.pptx,.ppt,.mp3,.mp4,.jpg,.png,.jpeg,application/pdf",
          ...
          success: function (file, response) {
            ...
            $(file.previewTemplate).append(
              '<span class="server_file" style= "display: none;">' +
                response +
                "</span>"
            );
            console.log($("#linkAttach").val());
            if ($("#linkAttach").val().length > 0) {
              $("#linkAttach").val($("#linkAttach").val() + "&" + response);
            } else {
              $("#linkAttach").val(response);
            }
