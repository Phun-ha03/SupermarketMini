...
function buildSegmentBar(element, options) {
    ...
    for (let item of options.data) {
        ...
        valueSpan.textContent = item.subTitle//`${item.value.toLocaleString("en-US")}`;
        ...
        if (item.title && item.title.length > 0) {
            ...
            div.title = `Năm ${item.title} tuổi nghỉ hưu là ${item.subTitle.indexOf('Th') >= 0 ? item.subTitle.replace('T ', ' Tuổi ').replace('Th', ' Tháng') : item.subTitle.replace('T', ' Tuổi') }`;
        }
